...
public class GeminiService {
    ...
    public GeminiResponse chat(String systemPrompt, List<AIMessage> conversationHistory, String userMessage) {
        if (!isAvailable()) {
            ...
            throw new DoAn.BE.common.exception.BadRequestException(
                    "AI service chưa được cấu hình. Vui lòng thiết lập GEMINI_API_KEY.");
        }
        ...
        try {
            ...
            handleApiError(e);
            throw new DoAn.BE.common.exception.BadRequestException("Lỗi khi gọi AI: " + e.getMessage());
        } catch (Exception e) {
    ...
    private Map<String, Object> buildRequestBody(String systemPrompt, List<AIMessage> conversationHistory,
            String userMessage) {
            ...
            if (response.containsKey("promptFeedback")) {
                ...
                if (blockReason != null) {
                    ...
                    return new GeminiResponse(
                            "Xin lỗi, yêu cầu của bạn không thể được xử lý. Vui lòng thử lại với nội dung khác.", 0);
                }
            }
    ...
    private void handleApiError(WebClientResponseException e) {
        ...
        if (statusCode == 400) {
            if (errorBody.contains("API_KEY_INVALID")) {
                throw new DoAn.BE.common.exception.BadRequestException(
                        "API key không hợp lệ. Vui lòng kiểm tra GEMINI_API_KEY.");
            }
        } else if (statusCode == 429) {
            throw new DoAn.BE.common.exception.BadRequestException("Đã vượt quá giới hạn API. Vui lòng thử lại sau.");
