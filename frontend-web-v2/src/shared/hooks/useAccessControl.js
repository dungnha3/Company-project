...
export const useAccessControl = () => {
     ...
     * @param {string} permissionKey - Mã quyền (VD: 'PROJECT.CREATE', 'HR.VIEW_LIST')
     * @returns {boolean}
     */
    const hasPermission = (permissionKey) => {
        ...
        if (currentWorkspace.role === 'OWNER' || currentWorkspace.role === 'COMPANY_ADMIN') return true;

        const perms = currentWorkspace.permissions;
        if (!perms) return false;

        // Map permissionKey string to UserPermissions object property
