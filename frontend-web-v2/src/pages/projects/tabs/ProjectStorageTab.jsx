...
export default function ProjectStorageTab({ projectId, issueId: initialIssueId }) {
    ...
    const deleteMutation = useMutation({
        ...
        onError: () => toast.error('Không thể thực hiện xóa')
    });
    ...
    const getFileIcon = (contentType, fileName) => {
        if (contentType === 'folder') {
            return 'fa-folder text-amber-500 text-xl';
        }
        const ext = fileName.split('.').pop()?.toLowerCase();
        switch (ext) {
            case 'pdf':
                return 'fa-file-pdf text-rose-500 text-lg';
            case 'doc':
            case 'docx':
                return 'fa-file-word text-blue-500 text-lg';
            case 'xls':
            case 'xlsx':
                return 'fa-file-excel text-emerald-600 text-lg';
            case 'ppt':
            case 'pptx':
                return 'fa-file-powerpoint text-orange-500 text-lg';
            case 'png':
            case 'jpg':
            case 'jpeg':
            case 'gif':
            case 'svg':
                return 'fa-file-image text-violet-500 text-lg';
            case 'zip':
            case 'rar':
            case '7z':
                return 'fa-file-zipper text-amber-600 text-lg';
            case 'txt':
                return 'fa-file-lines text-slate-500 text-lg';
            default:
                return 'fa-file text-gray-400 text-lg';
        }
    };
    ...
    if (!driveStatus?.connected) {
        return (
            <div className="card p-12 text-center bg-white border border-gray-100 rounded-xl shadow-sm">
                ...
                <p className="text-gray-500 mb-6 max-w-md mx-auto text-sm leading-relaxed">Quản trị viên cần kết nối Google Drive trong phần Cài đặt Công ty để sử dụng tính năng lưu trữ tài liệu.</p>
            </div>
        );
    }
        ...
        <div className="card p-6 min-h-[520px] bg-white border border-gray-100 rounded-2xl shadow-sm flex flex-col">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            className="px-4 py-2 border border-gray-200 hover:border-gray-300 text-gray-700 rounded-xl text-sm font-semibold flex items-center gap-2 hover:bg-gray-50 transition-all shadow-sm"
                        >
                        ...
                        <button
                            ...
                            disabled={uploading}
                            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-semibold flex items-center gap-2 transition-all shadow-md shadow-indigo-100 disabled:opacity-50"
                        >
                            {uploading ? (
                ...
                {breadcrumbParts.map((part, idx) => (
                    <div key={idx} className="flex items-center gap-2 shrink-0">
                        ...
                        <button
                            ...
                            className={`font-semibold transition-colors ${idx === breadcrumbParts.length - 1 ? 'text-gray-700 pointer-events-none' : 'text-indigo-600 hover:text-indigo-800'}`}
                        >
                            {part}
                        </button>
                    </div>
            ...
            {isLoading ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center py-16 border-2 border-dashed border-gray-100 rounded-2xl bg-gray-50/30">
                    ...
                    <p className="text-xs text-gray-400 mt-1">Hãy tải file lên hoặc tạo các thư mục con.</p>
                </div>
            ) : (
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3.5 text-gray-600 text-xs font-medium">
                                            {item.uploadedBy?.fullName || 'Người dùng'}
                                        </td>
